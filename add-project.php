...
$alert = "";
	if (isset($_POST['saveproject'])) {
		...
		$nvol = htmlspecialchars($_POST['numofvol']);
		...
		include_once("config.php");
		if (empty($title) || empty($dur) || empty($date) || empty($time) || empty($loc) || empty($nvol) || empty($des)){
			global $alert;
			$alert = "<div class=".'"p-3 mb-2 bg-danger text-white"'.">Please fill up the form properly.</div>";
				...
				if (empty($file)) {
					$file = "No file added";
					    ...
					    $sql = "INSERT INTO projects(title,duration,starting_date,starting_time,location,num_of_volunteers,description,files) VALUES('$title','$dur','$date','$time','$loc','$nvol','$des','$file')";
			...
			<div class="container mb-5">
				...
				<form method="post" action="add-project.php">
					...
					<div class="">
						...
						<div class="form-group row">
							<label for="" class="col-sm-3 col-form-label">Number of Volunteers</label>
							<div class="col-sm-9">
								<input id="pj-volun" type="number"value="1" class="form-control" name="numofvol" required>
							</div>
						</div>
						<div class="form-group row">
						...
						<div class="text-center mt-5">
							<input type="submit" id="submit-btn-pj" onclick="pjSubmit()" class="btn btn-outline-success btn-lg btncmn" name="saveproject" value="Save Project" >
						</div>
					</div>
				</form>
			</div>
