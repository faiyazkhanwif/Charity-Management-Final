...
if (isset($_POST['update'])) {
		...
		if (empty($volname) || empty($volnric) || empty($volage) || empty($volnationality) || empty($volphone) || empty($volemail) || empty($voladdress)){
			global $alert;
			$alert = "<div class=".'"p-3 mb-2 bg-danger text-white"'.">Please fill up the form properly. Information not be updated</div>";
		} else if(!filter_var($volemail, FILTER_VALIDATE_EMAIL)){
			global $alert;
			$alert = "<div class=".'"p-3 mb-2 bg-danger text-white"'.">Please fill up the form properly. Enter a valid email.</div>";
		} elseif (!filter_var($volphone, FILTER_VALIDATE_INT)) {
			global $alert;
			$alert = "<div class=".'"p-3 mb-2 bg-danger text-white"'.">Please fill up the form properly. Enter a valid Phone number.</div>";
		} elseif (!filter_var($volage, FILTER_VALIDATE_INT)) {
			global $alert;
			$alert = "<div class=".'"p-3 mb-2 bg-danger text-white"'.">Please fill up the form properly. Enter valid age.</div>";
		} else {
